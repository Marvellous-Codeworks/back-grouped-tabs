...
1. Finds every tab that belongs to **any** tab group and is actually stuck — either natively discarded by Chrome (`discarded` state) or rendered as a blank `chrome://newtab/` or `about:blank` page (the typical symptom of the Chrome session-restore bug). Tabs that are already correctly loaded, and the tab you are currently looking at, are never touched.
2. For each stuck tab:
...
5. Shows a badge on the toolbar icon: an amber `...` while running, then a green count of how many tabs were fixed (or red if zero).
